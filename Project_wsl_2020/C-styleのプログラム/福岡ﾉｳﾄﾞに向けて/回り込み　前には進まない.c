...
void user_main(void)
{
  ...
  while (TRUE) {
    if (get_pixydat_index(1) == 0) {
      ...
      if (get_pixydat_x(1) < (PIXY_val + 160)  &&  get_pixydat_x(1) > (160 - PIXY_val)) {
        gV[VAR_G] = GyroMoterSpeed * (160 - get_pixydat_x(1)) / PIXY_val;
      } else {
        if (get_pixydat_x(1) > 160) {
          gV[VAR_G] = - GyroMoterSpeed;
        } else if (get_pixydat_x(1) < 160) {
          gV[VAR_G] = GyroMoterSpeed;
        } else {
          gV[VAR_G] = 0;
        }
    ...
    if (get_timer(T1) < 400L) {
      ...
      } else if (sub_io_get_sensor(3, CN3) < 511) {
        set_Led(0, LED_ON);
        RfLb_X = MoterSpeed;  	// 後ろに行く
        LfRb_Y = MoterSpeed;  	// 後ろに行く
      } else {
        set_Led(0, LED_OFF);
      }
