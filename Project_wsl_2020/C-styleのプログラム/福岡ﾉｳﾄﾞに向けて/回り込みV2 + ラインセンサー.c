...
#include "D_EIO.h"
//--------------------------------------------------------------------------------
// Program Name : 回り込みV2 + ラインセンサー.C
//--------------------------------------------------------------------------------
void user_main(void)
{
  const int MoterSpeed = 45;  	// IRボールを追うときの動く速さ
  ...
  while (TRUE) {
    if (gAD[CN6] > 368) {
      ...
      LINEedge = gAD[CN10] * 18 / 17;  	// IR角度を代入
      if (gAD[CN5] < 511) {
        motor(-50, -50);
        gV[VAR_G] = 0;  	// 静止する
        RfLb_X = 0;  	// 静止する
        ...
        RfLb_X = cos(gV[VAR_L] * 3.1415 / 180) * MoterSpeed;  	// 白線から離れる動き
        LfRb_Y = sin(gV[VAR_L] * 3.1415 / 180) * MoterSpeed;  	// 白線から離れる動き
      } else if (gAD[CN1] < 10) {
        motor(-50, -50);
