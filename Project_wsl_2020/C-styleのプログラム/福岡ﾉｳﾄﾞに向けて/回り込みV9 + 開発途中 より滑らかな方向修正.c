//--------------------------------------------------------------------------------
// Title: Main - [ 回り込みV9 + 開発途中 より滑らかな方向修正 ]
...
#include "D_EIO.h"
//--------------------------------------------------------------------------------
// Program Name : 回り込みV9 + 開発途中 より滑らかな方向修正.C
//--------------------------------------------------------------------------------
void user_main(void)
{
  ...
  while (TRUE) {
    ...
    if (get_timer(T1) < 300L) {
      ...
      set_Led(0, LED_ON);
      clr_timer(0);
      RfLb_X = -MoterSpeed;  	// 右後に行く
      LfRb_Y = -MoterSpeed;  	// 右後に行く
    } else if (sub_io_get_sensor(1, CN8) < 511) {
      set_Led(0, LED_ON);
      clr_timer(0);
      RfLb_X = MoterSpeed;  	// 右前に行く
      LfRb_Y = -MoterSpeed;  	// 右前に行く
    } else if (sub_io_get_sensor(1, CN6) < 511) {
      set_Led(0, LED_ON);
      clr_timer(0);
      RfLb_X = -MoterSpeed;  	// 左前に行く
      LfRb_Y = MoterSpeed;  	// 左前に行く
    } else if (sub_io_get_sensor(1, CN7) < 511) {
      set_Led(0, LED_ON);
      clr_timer(0);
      RfLb_X = -MoterSpeed;  	// 左後に行く
      LfRb_Y = -MoterSpeed;  	// 左後に行く
