//--------------------------------------------------------------------------------
// Title: Main - [ 回り込みV２+ 開発途中 より滑らかな方向修正 - コピー ]
...
void user_sub_1(void)
{
  ...
  int IRdist = 5;
  while (TRUE) {
    Weight_X = gAD[CN4] * 1 * IRdist + gAD[CN5] * 2 * IRdist + gAD[CN6] * 3 * IRdist;
    Weight_X /= gAD[CN4] + gAD[CN5] + gAD[CN6];
    Weight_Y = gAD[CN4] * 1 * IRdist + gAD[CN5] * 2 * IRdist + gAD[CN6] * 1 * IRdist;
    Weight_Y /= gAD[CN4] + gAD[CN5] + gAD[CN6];
    Weight_X *= 1000; 
    Weight_Y *= 1000;
    Weight_angle = atan(Weight_Y / Weight_X) * 1000;
    gV[VAR_A] = Weight_X;
    gV[VAR_B] = Weight_Y;
    gV[VAR_C] = Weight_angle;
    printf("Weight_X : %4ld , Weight_Y : %4ld , Weight_angle : %4ld\r\n",gV[VAR_A],gV[VAR_B],gV[VAR_C]);
    if (Weight_angle > 560  &&  Weight_angle < 580  &&  (gAD[CN2] < 20 || gAD[CN2] > 340)) {
      gPwm[0] = 0x32 | 0x80;
      gPwm[1] = 0x32;
      gPwm[2] = 0x32 | 0x80;
      gPwm[3] = 0x32;
...
//--------------------------------------------------------------------------------
// Program Name : 回り込みV２+ 開発途中 より滑らかな方向修正 - コピー.C
//--------------------------------------------------------------------------------
void user_main(void)
{
  ...
  while (TRUE) {
    ...
    if (get_pixydat_index(gV[VAR_P]) > 0  &&  gV[VAR_P] < 3) {
      ...
      if (gAD[CN1] < 180  &&  gAD[CN1] > 8) {
        gV[VAR_G] = -GyroMoterSpeed;
      } else if (gAD[CN1] >= 180  &&  gAD[CN1] < 352) {
        gV[VAR_G] = GyroMoterSpeed;
      } else {
        gV[VAR_G] = 0;
      }
    ...
    if (get_timer(T1) < 300L) {
      ...
      if (Weight_angle > 569  &&  Weight_angle < 580  &&  (IRedge < 20 || IRedge > 340)) {
        ...
        } else if (gAD[CN3] > 300  ||  (IRedge > 135  &&  IRedge < 225)) {
          set_Led(3, LED_ON);
          if (IRedge > 180) {
    ...
    pwm_out();
    gV[VAR_A] = IRedge;
    gV[VAR_B] = Weight_angle;
    printf("IRedge : %4ld , Weight_angle : %4ld\r\n",gV[VAR_A],gV[VAR_B]);
  }
}
//--------------------------------------------------------------------------------
