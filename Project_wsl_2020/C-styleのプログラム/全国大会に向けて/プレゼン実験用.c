...
void user_main(void)
{
  ...
  int Comeramode_IRvalue = 180;  	// カメラモードに切り替える範囲＊実験用
  ...
  while (TRUE) {
    ...
    if (get_pixydat_index(gV[VAR_P]) > 0 &&  gV[VAR_P] < 3 && (IRedge < Comeramode_IRvalue  ||  IRedge > 360 - Comeramode_IRvalue)) {
      if (gV[VAR_P] == 1) {
    ...
    if (get_timer(T1) < 300L) {
      ...
      if (gAD[CN5] < gAD[CN4]  &&  gAD[CN5] < gAD[CN6]) {
        ...
        } else if (gAD[CN3] > 300 && (gAD[CN5] > gAD[CN4]  ||  gAD[CN5] > gAD[CN6])) {
          if (IRedge > 180) {
