//--------------------------------------------------------------------------------
// Title: Main - [ 回り込みV1+ 開発途中 より滑らかな方向修正 ]
...
#include "D_EIO.h"
//--------------------------------------------------------------------------------
// Program Name : 回り込みV1+ 開発途中 より滑らかな方向修正.C
//--------------------------------------------------------------------------------
void user_main(void)
{
  ...
  while (TRUE) {
    ...
    if (sub_io_get_sensor(1, CN1) < 511) {
      set_Led(2, LED_ON);
      clr_timer(0);
      if (sub_io_get_sensor(1, CN2) < 511) {
        RfLb_X = 0;  	// 左後に行く
      ...
      set_Led(2, LED_ON);
      clr_timer(0);
      if (sub_io_get_sensor(1, CN3) < 511) {
      ...
      set_Led(2, LED_ON);
      clr_timer(0);
      if (sub_io_get_sensor(1, CN4) < 511) {
        RfLb_X = 0;  	// 右前に行く
