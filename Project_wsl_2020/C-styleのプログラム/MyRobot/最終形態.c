...
void user_main(void)
{
  ...
  while (TRUE) {
    ...
    IR_edge = gV[VAR_I] * 3.14 / 180;  	// IR角度を読む　　ラジアンに直す
    gV[VAR_I] = gAD[CN4];
    ...
    if (gAD[CN4] < 400) {
      ...
      if (IR_edge > ( IR_frontValue ) * 3.14 / 180  &&  IR_edge < ( 360 - IR_frontValue ) * 3.14 / 180) {
        IR_alpha = ( (IR_alpha_min - IR_alpha_max) / 32400 ) * gV[VAR_I] * gV[VAR_I];
        IR_alpha = IR_alpha - (IR_alpha_min - IR_alpha_max) / 90 * gV[VAR_I] + IR_alpha_min;
        if (IR_edge < ( 180 + 45 ) * 3.14 / 180) {
