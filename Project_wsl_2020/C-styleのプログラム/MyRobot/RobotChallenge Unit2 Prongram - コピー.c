//--------------------------------------------------------------------------------
// Title: Main - [ RobotChallenge Unit2 Prongram - コピー ]
...
#include "D_EIO.h"
//--------------------------------------------------------------------------------
// Program Name : RobotChallenge Unit2 Prongram - コピー.C
//--------------------------------------------------------------------------------
void user_main(void)
{
  double IR_motermax = 80;  	// 回り込む際のモータの最大出力結果
  double IR_frontValue = 20;  	// 前とみなす、角度の範囲（IR）
  double IR_front_number = 1.8;  	// ボールが前に近しいところにいるときの角度を微調整
  ...
  double Gyro_moter_min = 20;  	// ジャイロのモータの最大出力結果
  ...
  while (TRUE) {
    ...
    gV[VAR_I] = (gAD[CN3] - IR_edgeAdjust - (IR_edgeCentralValue - IR_edgeAdjust)) + 360;  	// IR角度の調整
    gV[VAR_I] = gV[VAR_I] % 360;
    IR_edge_true = gV[VAR_I] * 1.1;  	// IR角度を読む　　度数のまま
    gV[VAR_I] = (gAD[CN3] - IR_edgeAdjust) + 360 - 45;  	// IR角度の調整
    gV[VAR_I] = gV[VAR_I] % 360;
    IR_edge_move = gV[VAR_I] * 3.14 / 180 * 1.1;  	// IR角度を読む　　ラジアンに直す
    Gyro_edge = gAD[CN1] * 1.1;  	// ジャイロを読む
    ...
    } else if (gAD[CN3] > 400 ||  Gyro_edge > 50  &&  Gyro_edge < 310) {
    ...
    } else if (IR_edge_true < IR_frontValue * IR_front_number  ||  IR_edge_true  > (360 - IR_frontValue) - IR_frontValue * IR_front_number) {
      Line_timerAdd = Line_time;  	// ラインタイマー付属品の調整
