//--------------------------------------------------------------------------------
// Title: Main - [ RobotChallenge Unit2 Prongram - including Camera ]
...
#include "D_EIO.h"
//--------------------------------------------------------------------------------
// Program Name : RobotChallenge Unit2 Prongram - including Camera.C
//--------------------------------------------------------------------------------
void user_main(void)
{
  double IR_motermax = 80;  	// 回り込む際のモータの最大出力結果
  double IR_frontValue = 4;  	// 前とみなす、角度の範囲（IR）
  double IR_front_number = 1.01;  	// ボールが前に近しいところにいるときの角度を微調整
  double IR_edge_move = 0;  	// IRの角度
  double IR_edge_true = 0;  	// IRの角度
  double IR_edgeAdjust = 0;  	// IRの角度の調整用　０～３６０にする用
  double IR_edgeCentralValue = 0;  	// IRの角度の調整用　前の微調整用
  double IR_alpha_max = 70;  	// 回り込む際のボールに対して何度ずらして動くか（max）
  double IR_alpha_min = 70;  	// 回り込む際のボールに対して何度ずらして動くか（min）
  ...
  double Camera_xx;  	// ゴールのあるX座標
  double Camera_motermax = 20;  	// ゴールへ方向に向くための最大出力結果
  ...
  while (TRUE) {
    ...
    i2c_get_pixy();  	// Pixyから情報を得る
    Camera_xx = get_pixydat_x(1);  	// ゴールのあるX座標を求める
    ...
    } else if (Gyro_edge > 50  &&  Gyro_edge < 310) {
      motor(-50, -50);
