...
void user_main(void)
{
  ...
  double IR_edgeAdjust = -15;  	// IRの角度の調整用
  double IR_alpha_max = 60;  	// 回り込む際のボールに対して何度ずらして動くか（max）
  double IR_alpha_min = 50;  	// 回り込む際のボールに対して何度ずらして動くか（min）
  ...
  while (TRUE) {
    ...
    gV[VAR_I] =  gAD[CN2] + 360 + IR_edgeAdjust;  	// IR角度の調整
    gV[VAR_I] = gV[VAR_I] % 360;
    IR_edge_true = gV[VAR_I] * 1.1;  	// IR角度を読む　　度数のまま
    gV[VAR_I] =  gAD[CN2] + 360 + 45 + IR_edgeAdjust;  	// IR角度の調整
    gV[VAR_I] = gV[VAR_I] % 360;
    ...
    } else if (gAD[CN1] < 500  &&  IR_edge_true < 400) {
