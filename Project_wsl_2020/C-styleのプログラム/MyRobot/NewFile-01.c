...
void user_main(void)
{
  ...
  double IR_edgeAdjust = 135;  	// IRの角度の調整用　０～３６０にする用
  double IR_edgeAdjust2 = 45;  	// IRの角度の調整用　前の微調整用
  ...
  while (TRUE) {
    ...
    gV[VAR_I] = (gAD[CN1] - IR_edgeAdjust) + 360 - IR_edgeAdjust2;  	// IR角度の調整
    gV[VAR_I] = gV[VAR_I] % 360;
    IR_edge_true = gV[VAR_I] * 1.1;  	// IR角度を読む　　度数のまま
    gV[VAR_I] = (gAD[CN1] - IR_edgeAdjust) + 45 + 360 - IR_edgeAdjust2;  	// IR角度の調整
    gV[VAR_I] = gV[VAR_I] % 360;
