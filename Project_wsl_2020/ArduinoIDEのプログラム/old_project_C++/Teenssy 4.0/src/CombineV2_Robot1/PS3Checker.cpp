...
void App1_PS3Checker()
{
    while (!ControlSW_left && !ControlSW_front)
    {
        ...
        // NeoPixel・LCDを表示
        LCD_Show(); // LCDを表示
        if (LStick_Angle == -1 && RStick_Angle == -1)
        {
            for (int i = 0; i < 40; i++)
            {
        ...
        {
            LEDedge_NeoLight(RStick_Angle, 2, 240, 260, 100); // 右ステックの角度をNeoPixelで光らせる
            LEDedge_NeoLight(LStick_Angle, 2, 0, 20, 100);    // 左ステックの角度をNeoPixelで光らせる
