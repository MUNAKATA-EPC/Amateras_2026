...
// 割り込みによる更新を保証するため volatile を付与
// コート
static volatile bool _field_detected = false;
static volatile int _field_deg = 0xFF;
// 青ゴール
static volatile bool _blue_goal_detected = false;
static volatile int _blue_goal_deg = 0xFF;
static volatile float _blue_goal_dis = 0xFF;
// 黄ゴール
static volatile bool _yellow_goal_detected = false;
static volatile int _yellow_goal_deg = 0xFF;
static volatile float _yellow_goal_dis = 0xFF;

static PacketManager packet;       // パケットマネージャー
static IntervalTimer _openmvTimer; // バックグラウンド処理用タイマー

// 割り込みハンドラ：受信とデコードをバックグラウンドで実行
void FASTRUN openmvBackgroundUpdate()
{
    if (_serial == nullptr)
        return;
...
bool openmvInit(HardwareSerial *serial, uint32_t baudrate)
{
    ...
    _openmvTimer.begin(openmvBackgroundUpdate, 50000); // 50ms周期で監視
