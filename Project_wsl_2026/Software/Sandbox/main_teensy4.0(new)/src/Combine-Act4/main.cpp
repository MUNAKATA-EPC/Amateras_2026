...
/*LINEについて*/
#define LINE_SAME_ESCAPE_TIME 100 // 何ミリ秒同じ動きでラインから避けるか
#define LINEESCAPE_TIME 120       // ラインに触れてどのくらいラインから離れる動きをするか

// ↑ラインから離れる動きはLINE_SAME_ESCAPE_TIMEとLINEESCAPE_TIMEの合計 ”ではなく”
// まずLINE_SAME_ESCAPE_TIMEだけはなれて、そのあと、残りの時間、
...
void loop()
{
    ...
    SSD1306write(1, 0, 10, String("Y_edge : ") + String(Yellow_goal_deg), false);
    SSD1306write(1, 0, 20, String("Y_dist : ") + String(Yellow_goal_distance), false);
    SSD1306write(1, 0, 30, String("B_edge : ") + String(Blue_goal_deg), false);
    SSD1306write(1, 0, 40, String("B_dist : ") + String(Blue_goal_distance), false);
    SSD1306show(); // 表示
    ...
    else if (now_LINEtime <= LINE_SAME_ESCAPE_TIME)
    {
    ...
    {
        int Cam_front_deg; // 前をどの角度にするのか格納する変数

        if (CamToggle) // トグルによって青か黄色か変える
        {
            Cam_front_deg = Yellow_goal_deg;
        }
        else
        {
            Cam_front_deg = Blue_goal_deg;
        }
        ...
        else // それ以外
        {
            ...
            Cam_front_deg = 0; // 回り込むときは0度を前とする
        }

        Motor4pidControl(MotorPower, Cal_AroundIRedge, GYROMOTORMAX, BNO055deg, Cam_front_deg); // 回り込みさせる
