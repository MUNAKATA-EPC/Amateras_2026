...
void playDefenderVer2(Defender::Mode mode)
{
    ...
    if (defence_goal_detected && abs(fieldDeg()) <= 90)
    {
    ...
    // static bool defenced_flag = false; // 前回守備したかどうか
...
// #define Y
...
#else
                int abs_defence_ir_deg = abs(defence_ir_deg);
                if (irDis() < 600)
                {
                    if (abs_defence_ir_deg <= 10)
                    {
                        ir_defence_vec = ir_defence_vec * 0.0f; // 停止させる
                    }
                    else if (abs_defence_ir_deg <= 20)
                    {
                        ir_defence_vec = ir_defence_vec * (abs_defence_ir_deg - 10) / 10; // 停止させる
                    }
                }
                else
                {
                ...
                /*
                //// ボールの前でがたつくのを防止する処理 ////
                static Vector normal_vec;
                static unsigned int change_count = 0;
                static bool old_normal_flag = false;
                static Timer normal_timer, abnormal_timer;

                // 基準ベクトルの決定
                if (defenced_flag == false)
                {
                    normal_vec = final_defence_vec;
                    change_count = 0; // 守備開始時はリセット
                }

                // 現在のベクトルが「順方向(基準と同じ側)」か「逆方向」か判定
                // 基準ベクトルとのなす角が90度未満なら順方向
                bool normal_flag = abs(diffDeg(normal_vec.deg(), final_defence_vec.deg())) < 90;

                const unsigned int change_min_time = 10UL, change_max_time = 650UL; // 切り替え時間の最小・最大

                if (normal_flag != old_normal_flag)
                {
                    // 状態が切り替わった（反転した）瞬間
                    if (normal_flag)
                    {
                        // 逆方向から順方向に戻った時
                        if (abnormal_timer.msTime() >= change_min_time && abnormal_timer.msTime() <= change_max_time)
                        {
                            change_count++;
                        }
                        else if (abnormal_timer.msTime() > change_max_time)
                        {
                            change_count = 0;               // 長く逆方向にいたなら、それは「意図的な反転」なのでリセット[
                            normal_vec = final_defence_vec; // 基準ベクトルも更新
                        }
                        normal_timer.reset();
                    }
                    else
                    {
                        // 順方向から逆方向に切り替わった時
                        if (normal_timer.msTime() >= change_min_time && normal_timer.msTime() <= change_max_time)
                        {
                            change_count++;
                        }
                        else if (normal_timer.msTime() > change_max_time)
                        {
                            change_count = 0;               // 長く順方向にいたならリセット
                            normal_vec = final_defence_vec; // 基準ベクトルも更新
                        }
                        abnormal_timer.reset();
                    }
                }

                old_normal_flag = normal_flag;

                // 減衰処理
                if (change_count >= 3)
                {
                    // 3回以上の高速往復で減衰
                    float gain = 3.0f / (float)change_count;
                    final_defence_vec = final_defence_vec * fmaxf(gain, 0.4f);
                }
                    */
    ...
    /*
    // 前回守備をしたかどうか記録
    if (defence_ok && posi != LinePosi::Nai_line && irDetected())
    {
        defenced_flag = true;
    }
    else
