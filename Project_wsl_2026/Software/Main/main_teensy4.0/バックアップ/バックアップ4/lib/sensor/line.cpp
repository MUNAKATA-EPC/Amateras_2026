...
static MovementAverage _ave_xy[2] = {MovementAverage(5), MovementAverage(5)};
...
void lineUpdate()
{
    ...
    if (_ring_detected)
    {
        ...
        {
            _x = _ave_xy[0].add(_x * 100.0f / ringDetectedCount);
            _y = _ave_xy[1].add(_y * 100.0f / ringDetectedCount);
