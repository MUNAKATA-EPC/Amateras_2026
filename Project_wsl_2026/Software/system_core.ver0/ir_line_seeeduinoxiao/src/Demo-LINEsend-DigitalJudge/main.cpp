...
const int LINEjudgeVal = 600;                                                         // LINE素子の値が何以下だとラインがあると認識するか
...
double LINEedge;                            // LINEの角度を格納するための変数
...
void loop()
{
  ...
  for (int i = 0; i < LINEs; i++)
  {
    bool LINEcondition = MULTI16read(LINEpinNum[i]) > LINEjudgeVal; // "i"ピンのLINE素子に反応があるかどうか格納する変数

    LINEjudgeTotal += LINEcondition; // それぞれのLINE素子の結果を足し算していく

    LINEsumX += LINExNum[i] * LINEcondition; // 足し合わせていく
    LINEsumY += LINEyNum[i] * LINEcondition; // 足し合わせていく
  }
  ...
  {
    // LINEの角度を座標から計算
    LINEedge = degrees(atan2(LINEsumY, LINEsumX)) - 90.0; // アークタンジェントを使用
    if (LINEedge < 0)
      LINEedge += 360.0; // 0〜359の範囲に収める処理
    if (LINEedge >= 360.0)
      LINEedge -= 360.0; // 0〜359の範囲に収める処理
  }
  ...
  {
    LINEedgeSendData = String(int(LINEedge * 10));
    while (LINEedgeSendData.length() < 4)
    {
      LINEedgeSendData = "0" + LINEedgeSendData;
    } // 値を整形する
  }
  String SIDELINESendData = String(SIDELINE_RIGHT) + String(SIDELINE_LEFT) + String(SIDELINE_DOWN) + "0";

  Serial.println("a" + LINEedgeSendData + "b" + SIDELINESendData);
  Serial1.println("a" + LINEedgeSendData + SIDELINESendData);
