...
2016/4/4 change int value to double value for reference servomotor angle 
arranged by yu@furo
...
void VarSpeedServo::write(double value)
{  
  ...
  {  // treat values less than 544 as angles in degrees (valid values in microseconds are handled as microseconds)
	...
	//--------add
	value = fconstrain(value, 0, 180);
    value = fmap(value, 0, 180, SERVO_MIN(),  SERVO_MAX());    
...
*/
void VarSpeedServo::write(double value, uint8_t speed) {
  ...
  double degrees = value;
	...
	if (speed) {
		...
		if( (channel >= 0) && (channel < MAX_SERVOS) ) {   // ensure channel is valid
      		// updated to use constrain instead of if, pva
      		value = fconstrain(value, SERVO_MIN(), SERVO_MAX());
...
void VarSpeedServo::write(double value, uint8_t speed, bool wait) {
  write((int)value, speed);
...
double VarSpeedServo::read() // return the value as degrees
{
...
double fconstrain(double x, double min, double max)
{
	if(x>max)x=max;
	if(x<-min)x=min;	
	return x;
